...
#define PREV_E_mm_free(p) (GET(p) & (0x4))
...
#define GET_PREV_E_mm_free(bp) ((GET(HEAD(bp)))&0x4)

#define SET_PREVALLOC(bp) (GET(HEAD(bp)) |= 0x2)
#define RESET_PREVALLOC(bp) (GET(HEAD(bp)) &= ~0x2)
#define SET_PREV_E_mm_free(bp) (GET(HEAD(bp)) |= 0x4)
#define RESET_PREV_E_mm_free(bp) (GET(HEAD(bp)) &= ~0x4)
...
#define GET_PREV(bp) ( GET_PREV_E_mm_free(bp) ? ((void *)bp - M_BLOCK_SIZE ): ((void *)(bp) - SIZE((void *)(bp) - DSIZE)) )
...
int mm_init(void)
{
    ...
    /* Extend the empty heap with a mm_free block of CHUNKSIZE bytes */
...
extend_heap extends the heap with a new mm_free block. It is invoked when the
heap is initialized or mm_malloc or mm_realloc is unable to find a suitable fit.

before we extend the heap, check if the last block is mm_free, if yes, extend the
heap for size - GET_SIZE(prev) bits, At the end of the function, we call the coalesce
function to merge the two mm_free blocks and return the block pointer to the
merged blocks.*/
...
void *extend_heap(size_t size_)
{
    ...
    int size = size_;
    if( !PREVALLOC(end) ){
        if(PREV_E_mm_free(end)) size -= M_BLOCK_SIZE;
    ...
    size_t sign = 0 | GET_PREVALLOC(bp) | GET_PREV_E_mm_free(bp);
    PUT_HEAD(bp, PACK(size,sign) ); /* mm_free block header */
    PUT_FOOT(bp, PACK(size,sign) ); /* mm_free block footer */
...
After initializing, we use mm_malloc to allocate a block by incrementing the brk
...
1.When ptr==HEAP_NIL, call mm_malloc(size), when size==0, mm_free ptr.
...
strategies.:
    ...
    (2) else, check if the next space is mm_free, if so, coalesce the block and 
        the next block
*/
        ...
        if( newsize <= oldsize ){ /* newsize is less than oldsize */
            ...
            /* the next block is allocated */
                ...
                /* the remainder is greater than BLKSIZE */
                    size_t sign = 1 | GET_PREVALLOC(ptr) | GET_PREV_E_mm_free(ptr);
            ...
            else{ /* the next block is mm_free */
            ...
            size_t csize;
            // the next block is mm_free and the addition of the two blocks no less than the newsize
...
coalesce is to merge one mm_free block with any adjacent mm_free blocks and to
update binary tree's structure in time.
There are 4 possibilities:
Each possibility is listed below as case 0 to 3.
After coalescing, pointer returns to the big mm_freed block.*/
...
{	
	...
	if ( prev_alloc && next_alloc ){ /* Case 0 */
	    ...
	    size_t sign = 0 | GET_PREVALLOC(prev) | GET_PREV_E_mm_free(prev);
	    delete_node(prev);
		size += GET_SIZE( prev );
...
It is very likely to the operation on mm_realloc.*/
...
static void place(void *bp,size_t asize)
{
	...
	delete_node( bp );
	if((csize-asize) >= HNF){
	    size_t sign = 1 | GET_PREVALLOC(bp) | GET_PREV_E_mm_free(bp);
...
BINARY TREE :
    1.We must eventually find a fit block after searching the binary mm_free tree.
    2.We must choose the least size mm_free block compared with the requested size.
...
inline static void insert_node( void *bp )
{
    ...
    if(bpsize == 8){
        SET_PREV_E_mm_free( GET_NEXT(bp) );
...
delete_node is to delete a mm_free block from the mm_free-block binary tree. It also has
many possibilities.
...
inline static void delete_node(void *bp)
{
    ...
    if(bpsize == 8) {//if the block size = 8, remove it from the 8-byte list, O(n)
        RESET_PREV_E_mm_free( GET_NEXT(bp) );
...
static void printblock(void *bp) 
{
    ...
    printf("%p--prev_e_mm_free bit[%d] prevalloc bit[%d] allocated bit[%d] size[%d]\n",
     bp, !!(int)GET_PREV_E_mm_free(bp), !!(int)GET_PREVALLOC(bp), (int)GET_ALLOC(bp), (int)hsize); 
}
...
/*check a mm_free block in the BST
...
/*check a 16-bit mm_free block in the small list
...
/*check a 16-bit mm_free block in the mini list
    print the block and check the next node in the list
*/
static void checkminilist(void * bp){
    if(bp != HEAP_NIL){
