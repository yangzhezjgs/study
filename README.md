...
在C语言中，字符以`ascii`码的形式在内存中存储，每个符号占一个字节。字符串由一系列字符组成，以空字符`\0'`结尾(值为0)，在c语言中遇到空字符就意味到了结尾，不管后面是否还有内容，可以利用这个来分割字符串。

`*ptr = "Hello world!"`字符串的值是字符串的起始地址，将这个值赋值给`ptr`，字符串的值存储在只读存储区`.rodata`，无法修改字符串的值，这是一个常量指针。甚至可以用字符串和整数相加，得到的值是字符串起始地址之后的地址。

`char buf[] = "Hello world!"`是字符数组，字符串的值存储在数组中，可以修改。

*数组和数组名*

数组和指针是不同的，数组为数据分配了空间，指针只是为保存地址分配了空间。数组名是一个**指针常量，值为数组首元素的地址**，可以将数组名赋值给指针，不能修改数组名，不是左值。数组的下表访问其实是通过指针访问的，可以使用数组最后一个元素的下一个元素的地址，不可使用之前的。

*数组分配*

一般来说数组大小是固定的，也可以使用`malloc()`动态分配数组空间。C99支持以变量作为栈数组大小，所以可以使用函数传递的参数来分配数组，如`int a[n];`。

*多维数组*

C语言其实只有一维数组，多维数组也是一维数组，只是数组元素的类型为数组而已。多维数组需要指定后面的长度，因为需要确定数组元素的长度。
...
函数参数传递指针有两种方式：*和[]，这两种方式是一样的。当传递给函数时，数组和指针是一样的，**数组名会退化为指针**。当传递多维数组给函数时，需要指定后面的长度。
...
在结构体内部有个奇怪的用法，就是以`[]`或`[0]`声明的数组作为最后一个成员，这个成员的大小为0，但是如果给结构体分配了多余的空间，后面的空间会以该类型解释。举个例子：
```c
struct test {
    int count;
    int buf[];
...
1. 对变量而言：使用`static`声明的自动变量是静态变量，和全局变量类似，会被初始化为0，已显示初始化的存放在`.data`段，为初始化的存放在`.bss`段。静态变量在程序整个执行期间都存在，只初始化一次，变量的值在函数调用中保持延续性。形参不能声明为静态变量，因为形参必须分配在栈内，来支持递归调用。

2. 对函数而言：使用`static`声明的函数只在本文件内可见。通常在头文件中使用，声明时使用`static`，在定义时就不需要再用`static`，不过一般不需要把static函数的声明放在头文件中，直接在.c中定义即可。


*extern*

extern对函数而言是多余的，没有切实的作用。但对于变量而言，说明该变量是在别的地方定义的，一般是在别的头文件中，不会为该变量分配空间。要注意声明要和定义相匹配，指针和数组在这种情况是不同的，因为内存使用方式不同，通常用于在头文件中声明全局变量，然后在.c中定义，如果在头文件中定义可能会发生重定义。

*const*

const用于声明常量，只能在声明时对它进行初始化，不能修改它的值，**const修饰的是紧贴在它后面的类型**。
...
当然还有两个都是常量的情况，都不可以改变。`const`修饰的常量也是可以修改的，但必须通过别的指向相同地址的指针。最常见的是在函数形参中指定`const char *ptr`，防止在函数内部修改值。
...
sizeof(表达式)中的表达式不会被求值，只需要知道表达式的类型来获取长度。

sizeof(数组)会得到数组的空间大小。字符串会被视为数组，包括最后的空字符。
...
typedef用来给数据类型定义新的名字，声明的方式和普通声明相同，只是在前面增加了typedef。可以用来降低类型的复杂度。

*头文件*

预处理器会将#include包含的头文件替换为相应的文件内容。以`""`包围的搜索范围是现在当前文件内，之后按照特定的路径寻找。以`<>`包围的搜索范围为系统设置的路径。可以设置`C_INCLUDE_PATH`环境变量来增加搜索路径。通常会采用条件编译的方式，避免重复包含头文件。

要注意头文件的**相互包含**问题，需要去掉其中一个的头文件，使用提前声明。
...
**汇编**：将汇编语句转换为对应的机器指令。可以使用`gcc -c`产生目标文件`.o`。目标文件中包含机器指令代码、数据，还包含一些链接时需要的信息。目标文件将这些信息按不同的属性，以节的形式存储。有三种目标文件：
...
* 静态链接：将所有的目标文件的相似段合并，比如所有输入文件的`.text`合并到输出文件的`.text`。静态链接会造成内存和磁盘空间的浪费，因为会有重复的数据或代码，同时也会造成模块更新困难。

* 动态链接：动态链接用来解决静态链接的问题。静态连接是在装载前链接，动态链接是在装载的时候进行链接，在运行时才将它们链接在一起形成一个完整的程序。系统首先装载第一个目标文件，然后装载目标文件的依赖文件，当所有依赖关系满足时，开始链接工作。为了提高速度，会用**延迟绑定**的技术，在函数第一次被使用到的时候再进行符号解析、重定位等。当程序需要装载的目标文件依赖于之前已经装载过的文件时，只需要装载新的目标文件，就避免了代码或数据段的重复，节省了内存和磁盘空间。当使用的多个共享对象中有全局符号重复的情况时，只会装载第一个遇到的符号，之后的符号会被忽略。

使用`gcc -fPIC -shared`可以生成共享目标对象。`-shared`产生共享对象。`-fPIC`生成地址无关的代码。地址无关的代码使得多个正在运行的进程可以共享存储器中相同的库代码，实现这个目的要求库代码可以在任何地址装载和执行，这就是地址无关的代码。使用动态链接可以不需要重新编译就可以更新模块，只要在运行之前将相应的模块更新替换就行，在下一次运行时，新的目标文件会被自动装载到内存中并链接起来。
...
当需要运行可执行目标文件时，会通过调用**装载器**来运行它。装载器将可执行目标文件的代码和数据从磁盘拷贝到存储器中，创建虚拟地址空间，然后跳转到程序指令的入口处来运行程序：

1. 首先需要创建虚拟地址空间：创建页表结构，建立虚拟地址空间到物理空间的映射关系。

2. 读取可执行文件头，创建虚拟地址空间和可执行文件的映射关系。
...
* **栈**：栈是一个动态增长和收缩的段，从内存的高地址处向下增长，地址低的为栈顶。栈由**栈帧组**成，每调用一个函数，就会在栈上新分配一帧，当函数返回时就将此帧移去。

栈保存了下面信息：

1. 在栈最开始的地方，也就是地址最高的地方，保存了**命令行参数**和**系统环境变量**。

2. 函数实参和局部自动变量：在调用函数时自动创建，在函数返回时销毁。

3. 函数调用的信息：程序计数器和函数返回地址等，用于从函数调用中返回到之前状态。

* **堆**：在运行时**动态内存分配**使用的一块区域。堆顶部叫做"program break"，不是每次调用`malloc()`之类的函数都会增长堆的顶部，通常是直接增加一大块内存，之后在这一大块内存中分配需要大小的内存。

有下面几个注意点：

1. `malloc()`返回的是未经初始化的内存，`calloc()`返回初始化为0的内存。`malloc(0)`在Linux上返回一小块可以用`free()`释放的内存。

2. `realloc()`用于增加或减小内存的大小，增加的部分不会进行初始化。当紧邻原内存块后面的空闲内存大小不足时，`realloc()`会移动内存，任何之前指向该内存块内部的指针在调用`realloc()`之后不再可用，需要使用起始地址+偏移量来操作。`realloc(ptr, 0)`等效于`free(ptr)`+`malloc(0)`，而`realloc(NULL, size)`等效于`malloc(size)`。

3. 堆中的内存不会被自动释放，除非当进程终止时。当没有指针指向已分配的内存时，就造成了内存泄露。所以好习惯是当不再需要该内存块时就调用`free()`显示释放。

4. `alloca()`将从当前函数的栈帧上分配空间，分配的内存会随着栈帧的移除自动释放掉。
...
* 动态端口，通常也称为临时端口。动态端口通常被用来在主动发起连接时随机分配使用，在任何特定的连接外不具有任何意义。有一个特殊的端口号0，当绑定该端口号时，内核就分配一个临时端口号给用户进程。

#### *UDP*

**UDP提供的是无连接的、不可靠的、有消息边界的数据报通信。** UDP不提供可靠性：它把应用程序传给IP层的数据发送出去，但是不能保证它们到达目的地。UDP只是在IP协议上增加了两个东西：校验和和端口。因为UDP是无连接的，在一些情况下速度会比较快，比如如果应用程序只涉及到短的请求和应答，UDP性能就会比TCP好很多，因为没有建立连接和释放连接的开销。使用UDP的例子有：域名系统(DNS)、网络文件系统(NFS)和简单网络管理协议(SNMP)。

#### *TCP*

![](http://images0.cnitblog.com/cnitblog_com/wildon/544465b00200001s.png)
...
![三次握手](http://www.embedu.org/Column/images/Column700-1.jpg)

TCP建立连接需要发送三个段，所以也叫做**三次握手**。在建立连接的过程中，还有两个重要的选项会被交换：

1. 最大段大小(MSS)。用于向对端通告每个段所能发送的最大TCP数据量，目的是为了避免IP分片，通常大小为1460。(以太网MTU - IP头部 - TCP头部)

2. 窗口规模选项。用于通告对端当前缓冲区空间大小，对端不会发送比收到的窗口规模大的数据量，从而避免接受端发生溢出。
...
2. 为了防止已失效的连接请求报文段突然又传送到了服务端，因而产生错误，比如由于网络延迟直到连接已释放才发送到服务端的连接请求。如果不发送第三个数据段，服务器接收到失效的连接请求之后发送ACK，服务器会认为已经建立的连接，等待无效的客户端发送数据，导致资源的浪费和出错。

#### *释放连接*


![四次挥手](http://dl2.iteye.com/upload/attachment/0108/8315/51a9937d-3155-3a95-b853-97e8e20e758b.png)

释放连接需要交换4次，也叫做**四次挥手**。这是因为TCP连接是全双工的，需要每个方向单独进行关闭，当一端结束数据传输时，就发送**FIN**通知另一端。一个TCP连接在收到一个FIN后仍能发送数据，利用**半关闭(shutdown)**可以实现。

释放过程中，先发送**FIN**的一端执行的是**主动关闭**，当收到对端的**FIN**时，会进入一个特殊的状态`TIME_WAIT`。这个状态至少需要经过**2MSL(最大段生存时间)**后，才会到**CLOSED**状态。它主要有两个作用：

1. 可靠的实现TCP全双工连接的终止。因为最后发送的ACK有可能会丢失掉，对端会再次发送FIN，如果没有这个状态，会响应一个**RST**，导致对方进入错误状态而不是有序终止状态。当处于这个状态时，每收到一个段都会重启2MSL的计时器，目的是防止该ACK再次丢失。

2. 使老的重复TCP段在网络中消逝。因为数据有可能会重复发送，经过网络延迟直到连接关闭才到达目的地。如果这时没有相应的连接，TCP仅仅丢弃该数据并响应RST。然而，如果主机间又建立了一个具有相同IP地址和端口号的连接(称为前一个连接的**化身**)，就会认为该段是属于新连接的，导致连接被破坏。TIME_WAIT状态不允许发起新的化身，并且持续2MSL，就可以确保每成功建立一个连接，都不会受到之前连接的影响。不过也有例外，有些实现当新的SYN的序列号大于之前连接的结束序列号就会成功发起新的化身。

每个连接是由一个上述的五元组决定的，TIME_WAIT只要求**不能重新建立相同的五元组连接**，但是一般的实现都要求不能使用相同的**端口号**，条件更为苛刻。设置`SO_REUSEADDR`选项可以使用相同的端口号，但是不同使用相同的五元组。

#### *超时、重传与确认*

TCP是有状态的。内核保存着发送过的段，直到接收到对端发送的**ACK**才丢弃。当发送一个段时，会启动一个**定时器**，当定时器超时时还没有收到对端的ACK就会重传。重传不意味着数据没有发送到，也有可能是ACK丢失或者在网络中延时过长导致。如果一直收不到ACK，TCP会持续重传数据段，直到尝试一定次数才放弃，然后返回给应用进程一个错误，通常是`ETIMEOUT`。如果发生了可以被检测到的错误会直接返回错误，而不是超时重传。

TCP数据段是有序列号的，接受方接收到一个TCP段，会发送给发送方一个ACK段，ACK中的确认号是发送这个ACK的一端所期待的下一个序列号。

#### *数据传输*

![拥塞](http://www.cs.zju.edu.cn/network/courseware/netbase/chap6/images/fig6-29.jpg)

影响数据传输的主要有两方面：**网络**和**应用程序缓冲区空间**。

如果TCP突然有大量的段发送到网络中，由于网络的速率一般都不同，快速的网络的数据会积攒在路由器中，可能会导致路由器的缓冲空间耗尽，路由器丢弃数据，导致重传，进一步使网络拥挤。为了避免这个问题，TCP使用了**慢启动**和**拥塞避免算法**，并维护了一个**拥塞窗口(cwnd)**。

内核为每个套接字维护了两个缓冲区：**接收缓冲区**和**发送缓冲区**，每个TCP段都会通告对方当前接受缓冲区的可用大小，对端发送的数据不能超出该大小。与SYN一起发送的窗口大小通告了最大缓冲区大小。TCP使用了**滑动窗口**的技术，配合ACK来实现高效的数据传输。

发送窗口由对等方TCP使用，以预防应用程序发送的数据超过接收方TCP缓冲区；拥塞窗口由本方TCP使用，以预防应用程序超过网络所能承受的能力。任何时候TCP能够发送的最大数据量是发送窗口和拥塞窗口的最小值，以确保两种类型的流量控制都能有效。

#### *慢启动和拥塞避免*

![慢启动](http://img.my.csdn.net/uploads/201108/2/0_1312267750gJPx.gif)
...
当达到中间路由器的极限时，TCP段将被丢弃。拥塞避免算法是一种处理丢失TCP段的方法。拥塞避免算法使用了一个变量来控制：**门限阈值(ssthresh)**，当第一次拥塞时，该值被设置为当前拥塞窗口大小的一半，而拥塞窗口被重置为一个MSS，再次使用慢启动算法。当窗口大小到达阈值后，窗口不再呈指数级增长，每一次成功的传输会使拥塞窗口呈线性增长，直到到达接收方窗口大小。

**快速重传**和**快速恢复算法**是对拥塞避免算法的修改。当一端接受到3个或3个以上重复的ACK时，就重传丢失的数据报文段，而无需等待超时定时器溢出。这就是快速重传算法。接下来执行的不是慢启动算法而是拥塞避免算法，这就是快速恢复算法。由于TCP段到达时可能会出现乱序，如果几个序列号大的TCP段先到就会发送重复的ACK(原因在确认里提到过，ACK发送的是期望的下一个序列号)，但如果出现3个重复的ACK则很有可能该TCP段已经丢失了。

#### *滑动窗口*

![滑动窗口](http://img.my.csdn.net/uploads/201304/21/1366529169_2328.jpg)

在上面也提到了内核中的缓冲区概念。滑动窗口不是一个新的缓冲区，而是一种算法，用来高效的组织、协调TCP连接两端的缓冲区。

TCP连接的每一方都维护着一个**接受窗口**，窗口的最左边是期望接收的下一个序列号，最后边是能够接受的最大数据。当一个TCP段到达时，任何序列号超出接受窗口的数据将被丢弃，包括已接受的数据段(序列号在接受窗口左边)和未获得缓冲空间的数据段(序列号在窗口右边)。TCP段有可能是乱序到达的，就会缓冲在缓冲区中，滑动窗口也不移动，直到收到期望的数据段，这时会更新ACK，滑动窗口右移，接收后面的数据。当窗口为0的时候，有两种情况可以继续发送数据段：一是发送紧急数据；二是发送1个字节的数据段，让接收方通告下一个期望字节和窗口大小。这是由**TCP坚持定时器**决定的，当窗口大小为0时，就会启动该定时器，终止时发送一个一个探查段，查询是否有窗口大小更新，同时也**避免了窗口更新消息的丢失导致的死锁**。(因为TCP对于ACK是没有相应的ACK确认的)。
...
![](http://images.cnitblog.com/blog/228024/201306/08153946-05d0c7f220ec42da8f854f37c9719947.png)
...
![](http://images.cnblogs.com/cnblogs_com/xuanku/688508/o_server_tcp_simple.png)
...
```
`bind()`调用最常见的错误是`EADDRINUSE`(Address already in use)，这是因为绑定了重复的端口号导致的。在之前的网络协议里解释过`TIME_WAIT`状态和判定连接的五元组，当**处于TIME_WAIT状态时或者已经有相同端口号的连接时(例如，服务器接收连接，创建一个子进程处理连接，后来，服务器终止，子进程仍在运行)，不能绑定相同的端口号，设置`SO_REUSEADDR`选项，可以绑定相同的端口号，只要使用不同的IP地址即可。在Linux上设置该选项可以绑定多个不同的IP地址到同一个端口上，但是绑定了通配地址的端口只能有一个实例**。服务器通常都会在调用bind前设置该选项：
```c
#include <sys/socket.h>
...
![](http://img1.tuicool.com/UzeiMr3.png)
...
![](http://images.cnitblog.com/i/379121/201406/251857020336028.jpg)
...
当连接太频繁时，队列可能会被充满。**当已完成连接队列已满时，仍有客户连接进来，就会被存放在未完成连接队列，并向客户端发送SYN + ACK段，使客户端的`connect()`返回成功。当客户向服务器发送三次握手的最后一个ACK时，服务器会丢弃该ACK，因为已完成连接队列没有空间存放新的套接字。由于没有收到ACK，服务器会重传SYN + ACK，持续5次，之后等待已完成连接队列有空间或超时，若超时则将该套接字丢弃。客户端在`connect()`成功返回后，可以向套接字写不产生错误，这时由于连接没有建立，服务器不会发送ACK，会导致写的数据重传，当连接成功建立就会存储在接受缓冲区，如果超时，就会收到RST并返回错误。但如果没有发送数据，就不会产生RST，即使调用`read()`也不会产生错误，会一直阻塞下去**。
...
![](http://www.linuxidc.com/upload/2014_05/14052621273845.jpg)
...
`sendfile()`长和`TCP_CORK`选项一起使用，尤其在Web服务器当中，需要发送HTTP首部和页面数据。设置该选项候，所有的输出会**缓冲在一个TCP段中**，直到：
...
非阻塞的`accept()`在没有连接到达时返回`EAGAIN`，使用该版本可以预防阻塞和已连接套接字错误，因为有的实现忽略该错误。在Linux，使用非阻塞版本没有必要，详见`accept(2)`。
...
多路复用有两种触发机制，这两种机制很像电平，水平触发类似高低电平，边缘触发类似上升沿或下降沿。：

* **水平触发(LT)**： 当文件描述符上可以非阻塞地执行I/O系统调用，就通知就绪。也就是在接受缓冲区中有数据就通知可读条件；在输出缓冲区中有空间就通知可写条件。
...
当创建一个epoll实例时，内核在内存中创建了一个新的i-node并打开文件描述，`epoll_create()`返回的文件描述符就是指向这个文件描述的。内核中维护的数据结构是与打开的文件描述相关的：

* 当使用`epoll_ctl()`添加了一个元素到兴趣列表中，这个元素同时记录了文件描述符和监控的事件结构还有该**描述符对应的打开的文件描述**。当所有指向该打开的文件描述的文件描述符都被关闭后，这个打开的文件描述才会从epoll的兴趣列表中删除。可以添加复制的文件描述符到同一`epfd`。

* 如果`epfd`指向同一个打开的文件描述(例如通过`fork()`、`dup()`)，则兴趣列表也相同。通过其中一个`epfd`修改兴趣列表会影响所有的实例。也就是说，当有一个线程阻塞在`epoll_wait()`时，另一个线程添加兴趣列表会立即生效。
...
经常会出现多进程或多线程的服务器。如果有多个`accept()`阻塞在接受同一个监听套接字的连接，会唤醒所有的线程或进程，导致性能下降，这就是**惊群**。Linux2.6内核已经修复了这种现象，通过加锁只会唤醒一个调用。

更常见的是使用I/O多路复用技术。当有多个`epfd`等待相同的文件描述符就绪时也会发生惊群，可以通过抢锁的办法解决这个现象。在Linux4.5内核，提供了`EPOLLEXCLUSIVE`，可以在`epoll_ctl()`时与需要监听的事件相或来解决惊群现象。
