...
函数参数传递指针有两种方式：*和[]，这两种方式是一样的。当传递给函数时，数组和指针是一样的，**数组名会退化为指针**。当传递多维数组给函数时，需要指定后面的长度，以便为地址运算提供正确的单位长度。  
...
经常会出现多进程或多线程的服务器。如果有多个`accept()`阻塞在接受同一个监听套接字的连接(等待相同的fd进行读写一般没什么意义，所以最常见的在监听套接字)，会唤醒所有的线程或进程，导致性能下降，这就是**惊群**。Linux2.6内核已经修复了这种现象，通过加锁只会唤醒一个调用线程或进程。

更常见的是使用I/O多路复用技术。当有多个`epfd`等待相同的文件描述符就绪时也会发生惊群，比如将惊群从`accept`提前到了`epoll`，可以设置一把全局accept锁，只有最先抢到锁的进程或线程才注册listen_fd事件，也就不会注册相同的fd。在Linux4.5内核，提供了`EPOLLEXCLUSIVE`，可以在`EPOLL_CTL_ADD`时与需要监听的事件相或来解决惊群现象，只会唤醒设置了这个选项的并且阻塞在`epoll_wait`的线程。  
