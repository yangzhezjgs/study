...
有两种方式进行初始化，一种是长度未知`int a[] = {0};`会根据初始化元素个数确定数组大小；一种是长度固定，会按照顺序进行赋值，长度不够的补0，所以`int a[100] = {0};`就将数组初始化为0了。要注意边长数组不能初始化，无法确定数组长度。  
...
```  
包含柔性数组的结构体只能静态初始化，也就是只能是全局变量或静态变量时可以初始化，柔性数组还是柔性数组，和普通数组初始化不同。不过C11好像修改了，感觉最好还是不要用这个特性。  
  
*联合*  

联合使得一个变量可以合法地保存多重数据类型中任何一种类型的对象，空间为类型中最大的，只能以第一种类型的值初始化。  
...
* **堆**：在运行时**动态内存分配**使用的一块区域。堆顶部叫做"program break"，不是每次调用`malloc()`之类的函数都会增长堆的顶部，通常是直接增加一大块内存(虚拟内存页的倍数)，之后在这一大块内存中分配需要大小的内存。
